...
func TestTrunkCRUD(t *testing.T) {
	...
	_, err = trunks.Update(client, trunk.ID, updateOpts).Extract()
	if err != nil {
		t.Fatalf("Unable to update trunk: %v", err)
	}

	// Get subnet
	retrievedTrunk, err := trunks.Get(client, trunk.ID).Extract()
	if err != nil {
		t.Fatalf("Unable to get trunk: %v", err)
	}

	if retrievedTrunk.Name != updateOpts.Name {
		t.Fatalf("Trunk name was not updated correctly")
	}

	tools.PrintResource(t, retrievedTrunk)
}
...
func TestTrunkSubportOperation(t *testing.T) {
	...
	th.AssertDeepEquals(t, addSubportsOpts.Subports[0], subports[0])
	th.AssertDeepEquals(t, addSubportsOpts.Subports[1], subports[1])
